...
const SignInModal: React.FC = () => {
  ...
  const { isSignInModalOpen, closeSignInModal, signInMessage, signInModalCallbackUrl } = useAuthModal();
  ...
  const callbackUrl = signInModalCallbackUrl || "/";
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-sm w-full relative">
        ...
        <div className="text-center">
          ...
          {/* Use state from context for message */}
          {signInMessage && (
            <p className="mt-1 text-sm text-gray-600">{signInMessage}</p>
          )}
        </div>
        ...
        <div className="mt-6 space-y-4">
          ...
          <form onSubmit={handleCredentialSignIn} className="space-y-4">
           ...
           <div className="text-center text-sm text-gray-600">
                ...
                <Link href="/auth/signup" className="font-medium text-[#E65C1C] hover:text-[#FF8A50]">
                  Sign up
                </Link>
            </div>
        </div>
